...
for (drug_idx in 1:nrow(training.classes))
{
  ...
  for (cv_idx in seq(1:3)) {
    ...
    if (x.train_w_idx %>% length > 0)
    {
      ...
      #train model
      x.model <-
        elmtrain(x = training.train_elm_input,y = training.classes_train_cv,nhid=10,actfun="tansig")
      ...
      x.class_ag <-
        table(round(predict(
          x.model,training.validation_elm_input
  ...
  #test
  kaggle.predictions <- predict(x.cv_elm,kaggle.elm_input)
...
fptr <- file(description="./tansigmoidELM_out.csv",'w')
