...
for (drug_idx in 1:nrow(training.classes))
{
  ...
  for (cv_idx in seq(1:191)) {
    ...
    index <-
      sample(1:ncol(training.features_train),round(0.75 * ncol(training.features_train)))
    ...
    feat_lim <- cv_idx
    tree_lim <- c(50,100,200,400)[mod(cv_idx%/%8 + 1,4) + 1]
    gbm_dist <- sample(c("bernoulli","gaussian","huberized","adaboost"))[mod(cv_idx%/%8 + 1,4) + 1]
    ...
    if (x.all_features %>% length > 0)
    {
      ...
      #train model
      x.model <-
        gbm.fit(x = training.train_gbm_input,y = training.classes_train_cv,bag.fraction=.7,n.minobsinnode = 5,distribution = gbm_dist)
      ...
      x.class_ag <-
        table(round(predict(
          x.model,training.validation_gbm_input,n.trees=tree_lim
  ...
  #test
  kaggle.predictions <- predict(x.cv_gbm,kaggle.gbm_input,n.trees=tree_lim)
...
print(c("err:",x.cv_err,"feat_lim:",x.feat_lim,"tree_lim",x.tree_lim,"dist:",x.gbm_dist))
```
