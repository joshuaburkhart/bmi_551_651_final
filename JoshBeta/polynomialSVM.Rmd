...
for (drug_idx in 1:nrow(training.classes))
{
  ...
  for (cv_idx in seq(1:5)) {
    ...
    if (x.train_w_idx %>% length > 0)
    {
      ...
      #train model
      x.model <-
        svm(x = training.train_svm_input,y = as.factor(training.classes_train_cv),kernel="polynomial")
  ...
  #test
  kaggle.predictions <- predict(x.cv_svm,kaggle.svm_input)
...
fptr <- file(description="./polynomialSVM_out.csv",'w')
