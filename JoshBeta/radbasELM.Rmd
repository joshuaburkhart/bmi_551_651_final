...
for (drug_idx in 1:nrow(training.classes))
{
  ...
  for (cv_idx in seq(1:3)) {
    ...
    if (x.train_w_idx %>% length > 0)
    {
      ...
      #train model
      x.model <-
        elmtrain(x = training.train_elm_input,y = training.classes_train_cv,nhid=10,actfun="radbas")
...
fptr <- file(description="./radbasELM_out.csv",'w')
