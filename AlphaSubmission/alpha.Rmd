...
# gene id rows, cell line columns

subtypes <- t(read.table("../data/subtypes.txt",header=TRUE,sep="\t",row.names=1))
# subtype rows, cell line columns

training_set_answers <- t(read.table("../data/training_set_answers.txt",header=TRUE,sep="\t"))
# drug rows, cell line columns
...
classes_and_subtypes <- merge(x=subtypes,y=training_set_answers,all.y=TRUE) %>% merge(x=.,y=subtypes,all=TRUE)
