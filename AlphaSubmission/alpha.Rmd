...
training.classes <- t(read.table("../data/training_set_answers.txt",header=TRUE,sep="\t",check.names = FALSE))
...
training.features <- t(sae[colnames(training.classes),])
col_names <- names(training.features)
training.features[,col_names] <- lapply(training.features[,col_names] , numeric)

kaggle.features <- t(sae[setdiff(rownames(sae),colnames(training.classes)),])
kaggle.features[,col_names] <- lapply(kaggle.features[,col_names] , numeric)
```
...
training.classes_train <- training.classes[1,index]
training.features_test <- training.features[,-index]
training.classes_test <- training.classes[1,-index]
```
...
for(cv_idx in seq(1:3)){
  ...
  for (i in 2:nrow(training.features_train)) {
    x.w <-
      wilcox.test(as.numeric(training.features_train[i,]) ~ training.classes_train)
  ...
  training.features_train_w <-
    training.features_train[c(x.train_w_idx,1),] # adding row 1 (subtype)
