...
index <-
  sample(1:ncol(training.features),round(0.9 * ncol(training.features)))
...
for (cv_idx in seq(1:3)) {
  ...
  #train model
  x.model <-
    svm(training.svm_input,training.classes_train_cv,kernel = "linear")
  ...
  #test
  x.class_ag <-
    table(predict(x.model,training.svm_input),training.classes_validation_cv) %>%
    classAgreement()
